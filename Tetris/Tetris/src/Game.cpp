...
void Game::ProcessLockAndResolve()
{
    ...
    // 줄을 지우지 않은 T-Spin은 기존 B2B를 유지할 수는 있지만,
    // 그 동작만으로 새로운 B2B 체인을 시작하지는 않습니다.
    // 일반적인 무클리어 배치도 현재 B2B 상태를 그대로 유지합니다.
...
bool Game::TryRotateCurrentPiece(RotationDirection direction)
{
    ...
    // 현재 회전 데이터는 완전한 피벗 모델이 아니라 4x4 블록 배열 기반이라서,
    // 순수 SRS만 쓰면 벽에 붙은 상태의 일부 회전이 지나치게 빡빡해질 수 있습니다.
    // 그래서 벽에 닿아 있을 때만 최소한의 수평 보정을 추가로 확인합니다.
